...
├── lib/
│   ├── content/
│   │   ├── loader.ts       # getAllTrilhas, getAllConceitos, getBySlug (cached)
...
Theme colors are defined via CSS variables used throughout the app. Common colors include `background`, `foreground`, `primary`, `secondary`, `accent`, `muted`, `border`, and `card`.
...
Use a skill `/criar-trilha` ou crie manualmente `src/content/trilhas/<slug>.mdx` com o frontmatter acima. O arquivo é detectado automaticamente pelo loader, sem registro adicional.

### Criando um novo conceito (aula)

Use a skill `/criar-conceito` ou crie manualmente `src/content/conceitos/<slug>.mdx` com o mesmo frontmatter. Para que apareça em uma trilha, adicione o `slug` no campo `related` da trilha e inclua um `<ConceptCard>` no corpo MDX da trilha.
...
- Preserve the existing content-driven architecture unless the task explicitly asks for a structural change.
- Prefer small, targeted edits over broad rewrites, especially in MDX content files.
- Validate frontmatter and internal references whenever adding or renaming trilhas/conceitos.
- Use `"use client"` only when a component genuinely needs client-side behavior.
- Keep UI changes aligned with the existing visual language: Tailwind utilities, theme variables, and reusable content components.
