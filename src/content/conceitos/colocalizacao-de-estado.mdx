...
>
  <DecisionFlowItem
    condition="Só este componente?"
    solution="useState local"
  />
  <DecisionFlowItem
    condition="Irmãos próximos?"
    solution="Levantar ao pai mais próximo"
    detail="lifting state up"
  />
  <DecisionFlowItem
    condition="Uma subárvore inteira?"
    solution="Context API com Provider localizado"
  />
  <DecisionFlowItem
    condition="Dados do servidor?"
    solution="React Query / SWR"
    detail="não é estado, é cache"
  />
  <DecisionFlowItem
    condition="Realmente global?"
    solution="Zustand / Redux"
    detail="login, tema, carrinho"
  />
</DecisionFlow>
