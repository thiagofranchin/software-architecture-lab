...
<ProcessSteps
  variant="pratica"
  eyebrow="Processo prático"
  title="Sequência recomendada para refatorar com segurança"
  description="Pense no fluxo como uma rotina curta e repetível: proteção → mudança pequena → validação → commit. Cada passo existe para reduzir risco e facilitar reversão."
>
  <ProcessStep
    title="Confirme que os testes existem e estão passando"
    note="Se ainda não existem, escreva-os antes de continuar."
  />
  <ProcessStep
    title='Identifique um smell ou problema específico'
    note='Evite começar com algo difuso como "tudo aqui está ruim".'
  />
  <ProcessStep title="Aplique uma refatoração de cada vez" />
  <ProcessStep title="Rode os testes após cada passo" />
  <ProcessStep
    title="Se os testes ficarem vermelhos, pare no mesmo ponto"
    tone="attention"
  >
    Não avance enquanto a causa não estiver clara.
  </ProcessStep>
  <ProcessStep
    title="Faça commit quando os testes estiverem verdes"
    note="A mudança precisa fazer sentido sozinha e poder ser revertida sem arrastar o restante da sessão."
  />
</ProcessSteps>
